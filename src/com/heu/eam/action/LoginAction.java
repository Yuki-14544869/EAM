...
public class LoginAction extends BaseAction {
	...
	public String NormalLogin(HttpServletRequest request,
		...
		if(this.getServletContext().getAttribute("number")==null){
			...
			this.getServletContext().setAttribute("number", (int)this.getServletContext().getAttribute("number")+1);
		}
		
		try {
			map = ps.textLogin(username, pwd);
			...
			return "InterFace/login/login.jsp";
		}
		if (map == null) {
			request.setAttribute("msg", "用户名密码错误");
			return "InterFace/login/login.jsp";
			...
			response.sendRedirect("/EAM/menu");
			return null;
		}
	...
	public String FaceLogin(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		String img2 = request.getRealPath("InterFace/login/group1/") + "\\";

		// String img2 =
		// "C:/Users/new/Desktop/project/SRC/EAM/WebContent/InterFace/login/group1/";
		String path = request.getRealPath("InterFace/login/") + "\\"
		...
		BaseService<Person> bs = new BaseServiceImpl<>();
		try {
			Map<String, Object> map = bs.findById(Integer.parseInt(ID),
					Person.class);
		...
		if (f(message)) {
			...
			// response.setHeader("refresh", "0;URL=/EAM/menu");
			if(this.getServletContext().getAttribute("number")==null){
				this.getServletContext().setAttribute("number", 1);
			}else{
				this.getServletContext().setAttribute("number", (int)this.getServletContext().getAttribute("number")+1);
			...
			return "InterFace/login/login.jsp";
			/*response.setHeader("refresh", "0;URL=/InterFace/login/login.jsp");*/
		}
		return null;
	}
