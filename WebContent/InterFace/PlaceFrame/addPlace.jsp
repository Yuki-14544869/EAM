...
<body>
	<fieldset style="font-size:20px;">
		<legend style="font-size:40px;">添加场地信息</legend>
		<form action=<c:if test="${place=='true'}">/EAM/PlaceAction?method=updatePlace</c:if>
			<c:if test="${place!='true'}">/EAM/PlaceAction?method=addPlace</c:if> method="post"
			style="text-align: center;">
			<div  style="margin-top:20px;margin-left:15px;">
			&nbsp;编号:&nbsp;&nbsp;&nbsp; <input type="text" name="id" value="${place.id}" <c:if test="${isUpdate=='true'}">readonly</c:if>>&nbsp;
			<br /></div>
			<div style="margin-top:20px;margin-left:-10px;">&nbsp;&nbsp;名称:&nbsp;&nbsp;&nbsp; <input type="text" name="name"
				value="${place.name }">&nbsp;<br /></div>
				<div style="margin-top:20px;margin-left:-100px;">状态:&nbsp;&nbsp;&nbsp; <select
				name="state">
				<c:if test="${place=='true'}">
					<c:if test="${place.state=='1'}">
						<option selected="selected" value=1 style="font-size:20px;">已用</option>
						<option value=2  style="font-size:20px;">未用</option>
					</c:if>
					<c:if test="${place.state=='2'}">
						<option value=1  style="font-size:20px;">已用</option>
						<option selected="selected" value=2  style="font-size:20px;">未用</option>
					</c:if>
				</c:if>
				<c:if test="${place!='true'}">
					<option selected="selected" value=1  style="font-size:20px;">已用</option>
 ...
 <div style="margin-top:40px;"><input type="submit" style="font-size:20px;"
